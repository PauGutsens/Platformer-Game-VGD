...
bool Enemy::Update(float dt)
{
    if (IsOnScreen()) {
        ...
        // Keep the body in place when off-screen
        b2Vec2 velocity(0, 0);
...
bool Enemy:: IsOnScreen() {
    ...
    return (position.getX() >= -camera.x &&
        position.getX() < -camera.x + windowWidth &&
        position.getY() >= -camera.y &&
        position.getY() < -camera.y + windowHeight);
}